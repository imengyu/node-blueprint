<template>
  <div class="dock-tab">
    ...
    <div 
      ...
      @drop="onDrop"
    >
      <DynamicRender v-if="dockHost.hasSlot('renderTab')" :render="() => dockHost.renderSlot('renderTab', {
        dockData: dockData,
    ...
    <div class="dock-tab-content drag-target-host dock-ghost-area"></div>
  </div>
</template>

<script lang="ts">
import { defineComponent, inject, onMounted, PropType, ref, toRefs } from 'vue';
    ...
    function onDrop(ev: DragEvent) {
      ...
      if(currentIndexInPanel < 0 || currentIndexInPanel != dropTabMoveTargetIndex) {
        ...
        if(currentIndexInPanel >= 0) {
          oldParent.panels.remove(currentPanel);
          //当前拖动的元素移除掉之后，索引需要-1   
