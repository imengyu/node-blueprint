  ...
  >
    ...
    <template v-for="([ key, panel ]) in dockPanels" :key="key">
      ...
      >
        ...
        <slot v-else name="panelRender" :panel="panel" :key="key" />
      </div>
    </template>
    <!--顶层容器-->
    <DockSplit 
...
import { IKeyAnyObject } from '../Common/BaseTypes';
  ...
  methods: {
    ...
    onDrop(ev: DragEvent) {
      ...
      if((this.dropCurrentRegionDirection == 'float' && !this.allowFloatWindow)) 
        return; //不允许浮动窗口，则返回
      if(StringUtils.isNullOrEmpty(this.dropCurrentRegionDirection))
        return; //拖动区域是空的，返回
    ...
    onDragEnter(ev: DragEvent) {
      ...
      const host = this.host as HTMLElement;
      this.currentDragHostPos.x = HtmlUtils.getLeft(host);
      this.currentDragHostPos.y = HtmlUtils.getTop(host);
    ...
    flushDragDirectionRegion() {
      ...
      if(this.dropCurrentRegion != null) {
        ...
        let isCurrentViewNotRedundant = (!this.dropCurrentRegion.panels.contains(this.dropCurrentPanel as DockPanel) || this.dropCurrentRegion.panels.length > 1);
        ...
        this.showDropMiddle = !this.dropCurrentRegion.panels.contains(this.dropCurrentPanel as DockPanel);
    ...
    flushDragCurrentRegion(x : number, y : number) {
      ...
      const loopChilds = (dockData : DockData, region : Rect) : boolean => {
        ...
        if(gridLen > 0) {
          ...
          if (this.currentDragPanel 
            ...
            && !dockData.acceptPanelTags.contains(this.currentDragPanel.tag)) {
            //不可以接受，拒绝放置
            return false;
          }
      ...
      const isTabDrag = loopChilds(this.dockData as DockData, region);
    ...
    loadDockData(data : DockData, src : IDockGrid, direction : DockDirection) {
      ...
      data.size = src.size;
      data.name = CommonUtils.defaultIfUndefined(src.name, '');
      data.tag = CommonUtils.defaultIfUndefined(src.tag, '');
      data.alwaysVisible = CommonUtils.defaultIfUndefined(src.alwaysVisible, false);
      data.allowDep = CommonUtils.defaultIfUndefined(src.allowDep, true);
    ...
    addPanel(panel: IDockPanel, insertTo?: string|DockData) {
      if (StringUtils.isNullOrEmpty(panel.key)) {
        console.error(`[DockLayout] Panel uid: ${panel.uid} key is empty`);
        return;
      }

      if (this.dockPanels.has(panel.key)) {
        if (!insertTo || StringUtils.isNullOrEmpty(insertTo as string))
          return;
        //已存在，重新添加到指定的父级网格
