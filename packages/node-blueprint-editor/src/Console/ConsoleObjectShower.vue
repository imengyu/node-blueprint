...
export default defineComponent({
  ...
  methods: {
    ...
    loadObjectChilds() {
      ...
      switch(this.valType) {
        case 'Array': {
          let arr = this.$props.value as Array<SaveableTypes>;
        ...
        case 'Object': {
          let obj = this.$props.value as { [index: string]: SaveableTypes};
        ...
        case 'Set': {
          let arr = this.$props.value as Set<SaveableTypes>;
          ...
          for(let i = 0, c = arr.size; this.valChildNowIndex < c && i < 100; i++, this.valChildNowIndex++) 
            this.valChilds.push({ key: this.valChildNowIndex.toString(), val: vals[i] as unknown as SaveableTypes});
          this.valChildHasMore = this.valChildNowIndex < this.valChildAll - 1;
          break;
        }
        case 'Map': {
          ...
          for(let i = 0, c = keys.length; this.valChildNowIndex < c && i < 100; i++, this.valChildNowIndex++) {
            ...
            this.valChilds.push({ key: key + '', val: obj.get(key) as SaveableTypes });
          }
          this.valChildHasMore = this.valChildNowIndex < this.valChildAll - 1;
          break;
        }
