    ...
    onSplitDraggerMouseDown(e : MouseEvent, index : number) {
      if(e.button === 0) {         
        if(this.dockDatas && this.dockDatas.length > 0) {
          ...
          this.currentHostLeft = DockUtils.getLeft(this.host as HTMLElement);
          this.currentHostTop = DockUtils.getTop(this.host as HTMLElement);
        }
    ...
    onSplitDraggerMouseMove(e : MouseEvent) {
      ...
      if(this.currentSizeingPanel && this.currentSizeingPanelData &&  this.host && this.currentSizeingPanelDataNext) {
        if(this.direction === 'vertical') {
          //垂直
          let height = this.host.offsetHeight, minh = 80 / height * 100;
          let nh = (e.clientY - this.currentHostTop - (this.currentSizeingPanelData.startSize / 100 * height)) / height * 100;
