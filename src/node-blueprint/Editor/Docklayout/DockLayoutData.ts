  ...
  addGrid(grid: DockData, insertIndex?: number, forceSize = false): void {
    if (!this.grids.contains(grid)) {
      ...
      if (typeof insertIndex === 'number')
        this.grids.insert(insertIndex, grid);
      else
...
export class DockPanel implements IDockPanel {
  ...
  formJSON(json: IDockPanel): void {
    this.uid = CommonUtils.defaultIfUndefined(json.uid, this.uid);
    this.tag = CommonUtils.defaultIfUndefined(json.tag, this.tag);
    this.key = CommonUtils.defaultIfUndefined(json.key, this.key);
    this.title = CommonUtils.defaultIfUndefined(json.title, this.title);
    this.iconClass = CommonUtils.defaultIfUndefined(json.iconClass, this.iconClass);
    this.closeUnSave = CommonUtils.defaultIfUndefined(json.closeUnSave, this.closeUnSave);
    this.closeable = CommonUtils.defaultIfUndefined(json.closeable, this.closeable);
    this.customTab = CommonUtils.defaultIfUndefined(json.customTab, this.customTab);
    this.onClose = CommonUtils.defaultIfUndefined(json.onClose, this.onClose);
    this.onTabRightClick = CommonUtils.defaultIfUndefined(json.onTabRightClick, this.onTabRightClick);
    this.render = CommonUtils.defaultIfUndefined(json.render, this.render);
  }
}
