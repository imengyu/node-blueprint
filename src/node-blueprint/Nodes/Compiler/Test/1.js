...
_DEBUG_CONNECTOR.debugNode = function (context, graphUid, nodeUid) {
  ...
  if (this.debuggerPaused || this.debuggerStepMode) return true;
...
function makeDbgDefaults(dbg) {
  if (!dbg.nodeStack) dbg.nodeStack = [];

  if (!dbg.graphVariables) dbg.graphVariables = [];
...
function debugRunFunction(func, context, prepare = false) {
  ...
  _DEBUG_PROVIDER.newDebuggerRunnerContext(() => {
    ...
    if (!prepare) iterator = func(context);

    const debuggerRunnerContext = {
      state: "inactive",
      parent: context.parent
        ? _DEBUG_CONNECTOR.runnerContexts.get(context.parent.uid)
        : undefined,
      dbg,
      ...
      getLocalVariables() {
        ...
        while (currentContext.parent) {
          if (currentContext.dbg.graphVariables.length > 0) break;
...
function* main(context) {
  ...
  if (_DEBUG_CONNECTOR.debugNode(context, "6f696c72747a69", "6e6a6a33616836"))
    yield {
      type: _DEBUG_BREAK,
      uid: "6e6a6a33616836",
  ...
  if (_DEBUG_CONNECTOR.debugNode(context, "6f696c72747a69", "617462377a30746c"))
    yield {
      type: _DEBUG_BREAK,
  ...
  if (_DEBUG_CONNECTOR.debugNode(context, "6f696c72747a69", "68693773797973"))
    yield {
      type: _DEBUG_BREAK,
      uid: "68693773797973",
      graphUid: "6f696c72747a69",
  ...
  if (_DEBUG_CONNECTOR.debugNode(context, "6f696c72747a69", "6f64617a706579"))
    yield {
      type: _DEBUG_BREAK,
      uid: "6f64617a706579",
      graphUid: "6f696c72747a69",
